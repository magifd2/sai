...
```
イベント受信
  → staleイベントガード ← ts < startup_time − 30s なら破棄（インフラフィルタ）
  → 重複排除ガード      ← 再送イベントを破棄、キー: ts#channel_id#event_type
  → ACLチェック（セキュリティチェックは最初に必ず実行）
  → レートリミットチェック
  → 入力サニタイズ（Sanitizer）
  → Nonce生成・XMLカプセル化
  → プランナー（intent → action）
...
- **stale/重複排除ガードはインフラフィルタ — セキュリティチェックではない**
- **ACLチェックより前にセキュリティ処理を行わない**
