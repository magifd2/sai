...
SAIは保存済みメッセージへの変更を追跡します：

- **編集** — メッセージが編集されると、元のレコードはそのまま保持されます。編集後のテキスト（`[edited by <ユーザー名>]` プレフィックス付き）を含む新しいHOTレコードが追加され、以降のRAG検索で編集後の内容が取得できるようになります。
- **削除** — メッセージが削除された場合：
  - **HOT または PINNED レコード**: 即座にメモリから削除されます（embeddingも含む）。センシティブな内容が含まれていた可能性を考慮した仕様です。
  - **WARM または COLD レコード**: すでに要約済みのためレコードはそのまま残ります。サマリを遡って削除することはできません。
...
### 6. チャンネル・スレッドのオンデマンド要約

チャンネルやスレッドでの出来事をSAIに要約してもらうことができます：
...
SAIは指定されたスコープのメモリレコードをすべて取得して要約を生成し、対象となる記録の時間範囲を明記します：

```
## #general の要約 — 2026-03-20 10:00 UTC 〜 2026-03-24 15:30 UTC

• Q1リリース計画についてチームで議論した ...
• BobとAliceがAPI設計について合意した ...
```

> **注:** 24時間以上前のレコードはすでにLLMによるサマリ（WARM/COLD状態）になっている場合があります。長期間の要約は「サマリのサマリ」となるため、古い期間については精度が低下することがあります。SAIはこの点を回答に明記します。
